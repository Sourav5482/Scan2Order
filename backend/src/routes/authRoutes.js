...
router.post('/signup', async (req, res) => {
  try {
    const requiredCheck = hasRequiredFields(req.body, ['restaurantId', 'ownerName', 'email', 'password'])
    ...
    if (!restaurantId || !ownerName) {
      return res.status(400).json({
        success: false,
        message: 'restaurantId and ownerName cannot be empty',
      })
    }
